...
namespace MarcelloDB.Transactions
{
    internal class Journal
    {
        ...
        internal void Write (string collectionName, long address, ByteBuffer buffer)
        {
        ...
        {           
            ...
            foreach (var entry in journal.Entries.OrderBy(e => e.Stamp)) {
                ...
                writer.Write(entry.Address, this.Session.ByteBufferManager.FromBytes(entry.Data));
            }
            ClearJournal();
        }
        ...
        void PersistJournal(TransactionJournal transactionJournal){
            ...
            this.JournalWriter.Write(0, 
                new BufferWriter(this.Session, this.Session.ByteBufferManager.Create(sizeof(int)))
                    .WriteInt32(bson.Length).GetTrimmedBuffer()
            );
            this.JournalWriter.Write(sizeof(int), this.Session.ByteBufferManager.FromBytes(bson)); 
        }

        TransactionJournal LoadJournal(){
            var length = new BufferReader(
                this.Session.ByteBufferManager.FromBytes(
            ...
            if (length > 0)
            {
                ...
                return new BsonSerializer<TransactionJournal>()
                    .Deserialize(this.Session.ByteBufferManager.FromBytes(bytes));
            }
            return null;
        }
