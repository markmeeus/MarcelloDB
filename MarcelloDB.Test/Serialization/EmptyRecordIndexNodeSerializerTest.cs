...
namespace MarcelloDB.Test.Serialization
{
    ...
    public class EmptyRecordIndexNodeSerializerTest
    {
        ...
        {
            var node = new Node<EmptyRecordIndexKey>(12);
            ...
            node.EntryList.Add(new Entry<EmptyRecordIndexKey>{
        ...
        public void Empty_And_Full_Nodes_Have_Same_Byte_Size()
        {
            ...
            for(int i = 1; i <= Node<int>.MaxEntriesForDegree(12); i++) //max nr of entries for degree 12
            {
                node.EntryList.Add(
                    new Entry<EmptyRecordIndexKey>{
                        Key = new EmptyRecordIndexKey{S = 1, A = 1},
                        Pointer = i
                    }
                );
            }
            for(int i = 1; i <= Node<int>.MaxChildrenForDegree(12); i++) //max nr of childres for degree 12
            {
